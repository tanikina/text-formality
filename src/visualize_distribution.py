...
def create_visualization(dataset_name: str, setting: str, plot_path: str):
    ...
    # Reading data
    if setting == "llms":
        input_path_qwen = f"data/{dataset_name}/annotated/qwen7b_test.csv"
        df_qwen = pd.read_csv(input_path_qwen)
        y_true = np.array(df_qwen["gold"])
        y_pred_qwen = np.array(df_qwen["annotated"])

        input_path_llama = f"data/{dataset_name}/annotated/llama8b_test.csv"
        df_llama = pd.read_csv(input_path_llama)
        y_pred_llama = np.array(df_llama["annotated"])

        input_path_xlmr = f"data/{dataset_name}/annotated/finetuned_xlmr_test.csv"
        ...
        input_path_readability = f"data/{dataset_name}/annotated/avg_readability_score_test.csv"
        ...
        input_path_heylighen = f"data/{dataset_name}/annotated/heylighen_score_test.csv"
        df_heylighen = pd.read_csv(input_path_heylighen)
        y_pred_heylighen = np.array(df_heylighen["annotated"])

        input_path_lexical_diversity = f"data/{dataset_name}/annotated/lexical_diversity_test.csv"
        ...
        input_path_syntactic_complexity = (
            f"data/{dataset_name}/annotated/syntactic_complexity_test.csv"
        )
...
if __name__ == "__main__":
    ...
    create_visualization(args.dataset_name, args.setting, args.plot_path)
